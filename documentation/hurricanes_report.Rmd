...
The distribution of deaths is skewed right with most the most death counts occurring between 0 and 10. There are extreme observations having death counts over 200. 
...
Since the variance is much larger than the mean, the count data is over-dispersed. When count data is over-dispersed or under-dispersed, negative binomial regression can be used. Negative binomial regression is a generalization of Poisson regression and doesn't have the assumption of equidispersion. Negative binomial regression has an extra parameter, $\theta$ that can accommodate over-dispersion. Depending on the severity of over-dispersion, poisson regression might have inaccurate results, like smaller standard errors and confidence intervals, because the assumptions of the model are not met. 
